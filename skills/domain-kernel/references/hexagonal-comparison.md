...
| **Granularity** | Typically one hexagon per service or domain area (DDD: *bounded context*), often several use cases sharing one set of ports. | One kernel per domain, deliberately smaller — a kernel wrapping "scoring/trump/pots" as a single interface is flagged as too broad (see Mode C). |
